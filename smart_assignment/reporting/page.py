...
<svg viewBox="0 0 980 668" role="img" aria-label="Agentic workflow architecture diagram">
  ...
  <text x="823" y="182" text-anchor="middle" font-size="12.5" font-weight="700" fill="#0b2e59">☁️ Salesforce (CRM)</text>
  <text x="823" y="201" text-anchor="middle" font-size="10.5" fill="#5b6675">system of record · source of the prospect's details</text>
  <path d="M700,150 C700,80 700,35 658,35" fill="none" stroke="#1257a6" stroke-width="2" marker-end="url(#arw)"/>
  <text x="712" y="104" font-size="10" fill="#5b6675">prospect record</text>
...
(function () {
  ...
  // slot picker); fall back to the first prospect if it isn't in the set.
  var defaultKey = keys[0];
  for (var k = 0; k < keys.length; k++) {
    if ((DATA[keys[k]].name || '').indexOf('Woodlands') !== -1) { defaultKey = keys[k]; break; }
  }
  var defaultBtn = null;
  keys.forEach(function (key) {
    var b = document.createElement('button');
    ...
    chipsEl.appendChild(b);
    if (key === defaultKey) { defaultBtn = b; }
  });
  show(defaultKey, defaultBtn);
})();
"""

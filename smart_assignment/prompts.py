...
# The consolidated alternative to INSTRUCTION, used when
# Config.use_consolidated_pipeline_tool is on: root_agent gets ONE tool
# (assign_delivery_slot) that runs the whole deterministic chain.
#
# Note what disappears versus INSTRUCTION above: the "Golden rule", the
# "exactly three things end your turn" list, and steps 2-3. All of that text
# exists to stop the model halting between deterministic steps -- a failure mode
# that cannot occur when there are no intermediate steps to halt between. The
# behavior is enforced by the tool's shape rather than by prompt adherence, which
# is why this instruction is both shorter and more reliable.
...
Workflow for each prospect:
  1. As soon as the user has given you an address AND an order quantity in
     cases, call assign_delivery_slot with everything they've told you. It
     runs the whole workflow -- intake, geocoding, nearest routes, hard
     constraints, scoring, and the final decision -- in that one call.
     A preferred day/time is optional; pass it when they state one.
     If it returns {"ok": false}, relay the "error" message and ask the user
     for the missing or corrected value -- do not guess, and do not call any
     other tool until it returns {"ok": true}.
  2. Present the result. Do not stop at a one-liner -- when the result carries
     the structured fields, build your reply from them:
       - lead with "decision_summary" (the recommended route, day, and window);
       - give the main reasons from "primary_reasons" (each with its number);
       - state the "key_tradeoff" -- what this pick gives up versus the
         next-best option -- and name the "runner_up" so the user sees the
     ...
     You may lightly adapt wording, but never change a number, route, window, or
     the decision itself -- those came straight from the tool.
     The result's "workflow_steps" carries the geocoded location and every
     candidate route with its distance and feasibility; use it if the user asks
     what else was considered, or to add one line of context about the routes
     that were ruled out.

Escalation is AUTOMATIC -- never ask the user for permission to escalate, and
never end your turn with a question like "Would you like me to escalate this?".
The moment assign_delivery_slot returns "requires_human_review": true, you MUST
hand this off yourself in the SAME turn: present the escalation reason, then call
request_input to loop in a specialist. Do not just report the escalation and
stop, and do not wait for the user to say go ahead.
...
Revisions: if the user changes their mind about anything (a different
preferred day/time, a different order size, a corrected address), call
assign_delivery_slot again with ONLY the fields that changed -- everything
else already on file is kept automatically -- and present the new result.
...
# The consolidated-mode wording of the address-correction step. Identical policy
# to ADDRESS_RESOLUTION_GUIDANCE; only the "then continue the workflow" tail
# differs, because there is one tool to resume with instead of a three-step chain.
CONSOLIDATED_ADDRESS_RESOLUTION_GUIDANCE = """
Address correction: if assign_delivery_slot returns an error saying the address
...
real candidate matches and suggests the closest one -- it never invents an address.
 - If it returns "needs_confirmation": true, DO NOT proceed on your own. Show the
   ...
   confirm, call assign_delivery_slot again with the confirmed address.
...
# The consolidated-mode wording of the triage step (same policy, one tool name).
CONSOLIDATED_ESCALATION_TRIAGE_GUIDANCE = """
Escalation triage: whenever assign_delivery_slot returns
...
def build_instruction(
    ...
    include_address_resolution: bool = False,
    ...
    ``consolidated`` selects the one-tool workflow (see
    ``Config.use_consolidated_pipeline_tool``); each optional step has a matching
    consolidated wording so no variant ever names a tool the agent doesn't have.
    Default False reproduces the stepwise instruction byte-for-byte."""
    if consolidated:
        instruction = CONSOLIDATED_INSTRUCTION
        if include_address_resolution:
            instruction += CONSOLIDATED_ADDRESS_RESOLUTION_GUIDANCE
        if include_triage:
            instruction += CONSOLIDATED_ESCALATION_TRIAGE_GUIDANCE
        return instruction
